...
const { Line3, Vector3 } = require("threejs-math");

function getElevationOf2dPointFromTwo3dEdges(point, edge1, edge2) {
  const edge1Start = new Vector3(edge1[0][0], edge1[0][2], edge1[0][1]);
  const edge1End = new Vector3(edge1[1][0], edge1[1][2], edge1[1][1]);
  const edge2Start = new Vector3(edge2[0][0], edge2[0][2], edge2[0][1]);
  const edge2End = new Vector3(edge2[1][0], edge2[1][2], edge2[1][1]);

  // Create 3D points for the lines
  const line1_3d = new Line3(edge1Start, edge1End);
  const line2_3d = new Line3(edge2Start, edge2End);

  // Calculate the intersection point in 3D space
  const pointY3D = line1_3d.closestPointToPoint(
    line2_3d.closestPointToPoint(
  ...
  const pointElevation = pointY3D.y;
...
function cutEdges(floatPoints, edges, crossings, junctions, useColor) {
  ...
  for (i = 0; i < crossings.length; ++i) {
    ...
    const newFloatPoint = [ratToFloat(x[0]), ratToFloat(x[1])];
    const newFloatPointElevation = getElevationOf2dPointFromTwo3dEdges(
      newFloatPoint,
      [floatPoints[ee[0]], floatPoints[ee[1]]],
      [floatPoints[ef[0]], floatPoints[ef[1]]]
    );
    floatPoints.push([
      newFloatPoint[0],
      newFloatPoint[1],
      newFloatPointElevation,
    ]);
    ratPoints.push(x);
  ...
  for (var i = 0; i < numPoints; ++i) {
    ...
    if (j !== i) {
      ...
      floatPoints[j] = [
        ...
        Math.min(floatPoints[i][2], floatPoints[j][2]),
      ];
    }
  }
